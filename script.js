...
function startQuickTapMiniGame() {
  ...
  body.innerHTML = `
    <div class="quicktap">
      <div class="qt-info">Нажимай кнопку как можно быстрее за <span id="qtTime">8</span> сек</div>
      <div class="qt-targets" id="qtTargets"></div>
      <div class="qt-score">Счёт: <span id="qtScore">0</span></div>
    </div>`;
  quickTapState = { time: 8, score: 0, timer: null };
...
function startBowlingMiniGame() {
  ...
  body.innerHTML = `
    <div class="bowling">
      <div class="bw-info">Нажми чтобы запустить боулинг за <span id="bwTime">6</span> сек</div>
      <div class="bw-canvas" id="bwCanvas"></div>
      <div class="bw-score">Зачты: <span id="bwScore">0</span></div>
    </div>`;
  bowlingState = { time: 6, score: 0, timer: null, launches: 0 };
  ...
  launchBtn.onclick = ()=>{
    ...
    bowlingState.score += Math.floor(Math.random()*3)+2;
    document.getElementById('bwScore').innerText = bowlingState.score;
    launchBtn.style.transform = 'scale(0.8)';
    setTimeout(()=>{ launchBtn.style.transform = 'scale(1)'; }, 100);
  };
...
function startFlappyMiniGame() {
  ...
  flappyState = { time: 7, score: 0, timer: null, y: 150, vy: 0, obstacles: [], gameActive: true };
...
if (introVideo) {
  ...
  setTimeout(() => {
    const videoScreen = document.getElementById('videoScreen');
    if (videoScreen && videoScreen.classList.contains('active')) {
      hideVideoScreen();
    }
  }, 10000);
}
